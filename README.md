...
참고로 모든 소스코드는 영어에 약하므로 영어와 한국어을 같이 작성(번역기를 돌렸음)  
  
대상 MCU: STM32 계열 왜?  
20년전 학창시절에 어마어마한 컴파일툴 가격, JATG 디버깅툴, EVK 개발보드 가격과 더불어  
해당 자료(Datasheet)를 쉽게 접할 수 있는 부분이 Microchip PIC, STM32, Atmel 이였음.  
대다수 기업을 상대로 하기 때문에   
  
매번 느끼는 거지만 프로그램밍도 그렇고 처음엔 다 가볍고 빠름.  
문제는 편의 및 레거시(기존) 지원 때문에 점차 무거워지고 구조 변화가 어려움   
이게 반복됨 20년동안.  
  
언어도 수많은 언어가 C의 단점을 (전체 시나리오를 모르고) 새로운 걸 만들때  
생각 하지 못한 변수들로 인해 그 규칙은 점차 파괴됨(예외가 많아짐).  
  
언어는 자유도 만큼 그 책임은 개발자에 있습니다.  
이상 잡설이였습니다.  
