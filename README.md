# C언어 기반 Bare Metal 기반 오픈소스 입니다.
코드의 설계 방향성은 성능50% + 유지보수50% 생각하며 작성 합니다.

수십 us 차이가 별거 아니라고 생각 하겠지만 
이것에 수십만 수백만 반복시 또는 코드가 많아지면 그차이는 엄청 나게 차이 납니다.
그렇기 때문에 최대한 최적의 연산 과 처리를 고려 하면서 설계하도록 노력 했습니다.

일반 OS 소프트웨어 코드처럼 데이터 중심의 설계가 아닌
인터페이스의 속도 및 최적의 버퍼 용량등을 처리순서 기분을 중점으로 코드가 작성 되었습니다.
